...
const newEmailAddress = emailAddress.replace(emailAddress, username + "@" + newDomain); //use a string method and the oldDomain and newDomain variables to create the new email address as described in the starting comments
console.log(`The new email address is ${newEmailAddress}`);
const oldPassword = "johnsmith1987";
const tempPassword = oldPassword.slice(0, 5) + (9, 19);//use string methods and either concatenation or interpolation to generate the temporary password as described in the starting comments, you might find it useful to create another variable above this line to do this
